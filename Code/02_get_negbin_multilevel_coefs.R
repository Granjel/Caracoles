...
# see 00_test_statistical_model for a proof of concept of the model, using simulated data
...
# read data
competition <- read.csv(file = "data/competition_clean.csv",
...
# interaction.coefs <- expand.grid(all.years,all.ids,focal.sp,focal.sp)
# names(interaction.coefs) <- c("year","plot","focal","neighbour")
# interaction.coefs$magnitude <- NA_real_

lambda.coefs <- NULL
# lambda.coefs <- expand.grid(all.years,all.ids,focal.sp)
...
# main loop
# rescount <- 1
for(i.sp in 1:length(focal.sp)){# each focal sp
  
  print("*********************************")
  print(paste(date()," - starting focal sp ",focal.sp[i.sp],", model: ",fit.type,sep=""))
  
  sp.data <- subset(competition, focal == focal.sp[i.sp])
  # sp.data.long <- spread(sp.data,neighbour,number)

  my.data <- sp.data[,c("seed","year","plot",focal.sp)]

  # remove species that are never observed to co-occur
  for(sp in names(which(colSums(my.data[,focal.sp,drop=FALSE])==0))){
    my.data[,sp] <- NULL
  }
  ...
  if(fit.type == "homogeneous"){
    ...
    mean.lambdas$focal <- focal.sp[i.sp]
    mean.alphas$focal <- focal.sp[i.sp]
  ...
  # write.csv2(lambda.draws,paste("./results/temp/lambda_samples_",fit.type,"_",focal.sp[i.sp],".csv",sep=""),row.names = F)
  # write.csv2(alpha.draws,paste("./results/temp/alpha_samples_",fit.type,"_",focal.sp[i.sp],".csv",sep=""),row.names = F)

  # store species means
  # write.csv2(mean.lambdas,paste("./results/temp/lambda_coefs_",fit.type,"_",focal.sp[i.sp],".csv",sep=""),row.names = F)
  # write.csv2(mean.alphas,paste("./results/temp/alpha_coefs_",fit.type,"_",focal.sp[i.sp],".csv",sep=""),row.names = F)
...
all.positions <- expand.grid(year = all.years,plot = all.ids,sp = focal.sp)
...
alpha.positions <- expand.grid(year = all.years,plot = all.ids,
                               focal = focal.sp,
                               neighbour = focal.sp)
...
# # 
if(write.results){
      write.csv2(all.lambda,file = paste("Results/lambda_",fit.type,".csv",sep=""),row.names = F)
      write.csv2(all.alpha,file = paste("Results/alpha_",fit.type,".csv",sep=""),row.names = F)
}
...
# for(i.sp in 1:length(focal.sp)){# each focal sp
  ...
  #     my.num <- sum(comp.long$focal == focal.sp[i.sp] &
  ...
  #                                        lambda.coefs$sp == focal.sp[i.sp]]
  #     message(paste("sp ",focal.sp[i.sp],", plot ",all.ids[i.plot],", year ",all.years[i.year],", with n=",my.num," has lambda=",round(my.lambda,2),sep = ""))
  #     # if(is.na(my.lambda) & my.num < 5){
  #     #   message(paste("sp ",focal.sp[i.sp],", plot ",all.ids[i.plot],", year ",all.years[i.year],", with n=",my.num," not enough data",sep=""))
  #     # }
  #     # if(is.na(my.lambda) & my.num > 4){
  #     #   message(paste("sp ",focal.sp[i.sp],", plot ",all.ids[i.plot],", year ",all.years[i.year],", with n=",my.num," not calculated",sep=""))
  #     # }
  #   }
  # }
# }
